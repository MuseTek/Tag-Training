...
def get_sample_dimensions(path):
    melgram = load_melgram(path)
    print("   get_sample_dimensions: "+": melgram.shape = ",melgram.shape)
    return melgram.shape


# def encode_classes(filename, num_tags):  # makes a "one-hot" vector for each class name called
...
def build_free_sounds_dataset(path="Preproc/Train/", load_frac=1.0, batch_size=16, tile=False):

    mel_dims = get_sample_dimensions('/projects/MuseTek/preprocess/tag-process/Preproc/Train/kick00006.wav.npz')  # get dims of sample data file
    ...
    #now load dictionary that stores the numpy label arrays
    labels_dict = pickle.load(open("/projects/MuseTek/preprocess/tag-process/labels.p","rb"))

    sample_arr = next(iter(labels_dict.values()))
    sample_arr_len = sample_arr.shape[0]

    spectrogram_file_list = getListOfFiles('/projects/MuseTek/preprocess/tag-process/Preproc/Train')
    
    while True:
        #keep track of how many samples have been created
        samples_created = 0 
        X = np.zeros((batch_size, mel_dims[1], mel_dims[2], mel_dims[3]))
        Y = np.zeros((batch_size, sample_arr_len))
        batch_indexes = random.sample(range(0,len(spectrogram_file_list)), batch_size)
        
        files_to_retrieve = itemgetter(*batch_indexes)(spectrogram_file_list)
        load_count = 0
        for spect in files_to_retrieve:
            spect_path = path+'/'+spect
            melgram = load_melgram(spect_path)
            ...
            Y[load_count,:] = this_Y
            load_count += 1
            if (load_count >= batch_size):   # Abort loading files after last even multiple of batch size
                break
        yield X,Y

# test_gen = build_free_sounds_dataset(path='/projects/MuseTek/preprocess/tag-process/Preproc/Train')
# x,y = next(test_gen)
# pdb.set_trace()
